...
import com.featurevote.service.FeatureService;
...
public class FeatureController {
    ...
    @PostMapping("") // mapped to  -> /products/{productId}/features
    ...
    public String getFeature(@AuthenticationPrincipal User user,@PathVariable Long productId, @PathVariable Long featureId, ModelMap model){
        ...
        //TODO: handle the situation where we can't find the feature
        model.put("user", user);
    ...
    public String updateFeature(@AuthenticationPrincipal User user,Feature feature, @PathVariable Long productId, @PathVariable Long featureId){
        ...
        try {
            encodedProductName = URLEncoder.encode(feature.getProduct().getName(), "UTF-8").replace("+","%20");
